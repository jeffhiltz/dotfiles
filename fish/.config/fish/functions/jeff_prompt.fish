function fish_prompt --description 'Write out the prompt'
	...
	set -l arrows
	set arrows (set_color green) "❯" (set_color brgreen) "❯" (set_color white) "❯"

        set -l remote_host
        if set -q SSH_CLIENT
                set remote_host ' ' (set_color $fish_color_host) "$__fish_prompt_hostname" "$normal"
        end
	...
	echo -n -s $remote_host ' ' (set_color $color_cwd) (prompt_pwd) $normal (__fish_git_prompt) $normal $prompt_status "$mode_str" " " $arrows " "
end
